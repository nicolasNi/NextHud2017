...
import android_serialport_api.VoiceCommand;
...
//				}
    ...
    public static ArrayList<VoiceCommand> parseGrammar(String json, String engType) {
        StringBuffer ret = new StringBuffer();
        ArrayList<VoiceCommand> list=new ArrayList<VoiceCommand>();
        try {
            JSONTokener tokener = new JSONTokener(json);
            JSONObject joResult = new JSONObject(tokener);
            ...
            if ("cloud".equals(engType)) {
                for (int i = 0; i < words.length(); i++) {
                    ...
                    for(int j = 0; j < items.length(); j++)
                    {
                        ...
                        VoiceCommand cmd=new VoiceCommand();
                        cmd.Command=obj.getString("w");
                        cmd.Score=obj.getInt("sc");
                        list.add(cmd);
                    }
                }
